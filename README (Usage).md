...
* [SIMPLE USAGE EXAMPLES](#simple-usage-examples)
    ...
    * [Output and Organization](#output-and-organization)
...
This section provides straightforward examples for common download tasks. For advanced options, see [USAGE AND OPTIONS](#usage-and-options) and [FORMAT SELECTION](#format-selection).

## Basic Downloads

```bash
# Download a video (best quality by default - often downloads as .webm)
yt-dlp "https://www.youtube.com/watch?v=VIDEO_ID"

# Download from other supported sites (same syntax works for all)
yt-dlp "https://vimeo.com/VIDEO_ID"
yt-dlp "https://www.twitch.tv/videos/VIDEO_ID"
yt-dlp "https://twitter.com/user/status/TWEET_ID"

# Download as MP4 format (recommended - prefers H.264/AAC codecs for best compatibility)
yt-dlp -t mp4 "URL"

# Download as MP4 format (just remux container, doesn't change codec selection)
yt-dlp --remux-video mp4 "URL"

# Download as MKV format (using preset alias)
yt-dlp -t mkv "URL"

# Download as AVI format (requires re-encoding, slower)
yt-dlp --recode-video avi "URL"

# Download as MOV format
yt-dlp --remux-video mov "URL"

# Download to a specific folder
yt-dlp -P "C:/Downloads/Videos" "URL"

# Download as MP4 to a specific folder
yt-dlp -t mp4 -P "C:/Downloads/Videos" "URL"

# Download with a custom filename
yt-dlp -o "my_video.%(ext)s" "URL"

# Download with title as filename (default behavior)
yt-dlp -o "%(title)s.%(ext)s" "URL"
```

**Note on video formats:**
- `-t mp4` - **Recommended.** Preset alias that prefers H.264 video + AAC audio (widely compatible) and outputs MP4. Equivalent to: `--merge-output-format mp4 --remux-video mp4 -S vcodec:h264,acodec:aac`
- `-t mkv` - Same idea but outputs MKV container
- `--remux-video FORMAT` - Fast, just changes the container without re-encoding (no quality loss). Doesn't affect which codecs are downloaded. Works for: mp4, mkv, mov, webm, flv, avi
- `--recode-video FORMAT` - Slower, fully re-encodes the video (may reduce quality). Use only when remux isn't possible

## Audio Extraction

```bash
# Extract audio only (best available audio quality)
yt-dlp -x "URL"

# Extract and convert to MP3
yt-dlp -x --audio-format mp3 "URL"

# Extract and convert to MP3 (using preset alias - same as above)
yt-dlp -t mp3 "URL"

# Extract audio with specific quality (0=best, 10=worst)
yt-dlp -x --audio-format mp3 --audio-quality 0 "URL"

# Other audio formats: aac, flac, m4a, opus, vorbis, wav
...
# Download a specific format by format ID (from -F output)
yt-dlp -f 22 "URL"

# Download best MP4 format
yt-dlp -f "bv*[ext=mp4]+ba[ext=m4a]/b[ext=mp4]" "URL"

# Download best format up to 720p resolution
yt-dlp -S "res:720" "URL"

# Download best format up to 1080p
yt-dlp -S "res:1080" "URL"

# Download smallest file size
yt-dlp -S "+size,+br" "URL"

# Force MP4 container (using preset alias)
yt-dlp -t mp4 "URL"

# Force MKV container (using preset alias)
yt-dlp -t mkv "URL"
```
...
# Download only specific items from playlist (items 1-5)
yt-dlp -I 1:5 "PLAYLIST_URL"

# Download only the first 10 videos
yt-dlp -I :10 "PLAYLIST_URL"

# Download videos 5 through 10
yt-dlp -I 5:10 "PLAYLIST_URL"

# Download only the video, NOT the playlist (when URL refers to both)
yt-dlp --no-playlist "URL"

# Download playlist in reverse order
yt-dlp -I ::-1 "PLAYLIST_URL"

# Download playlist videos into separate folders by playlist name
...
# Download subtitles in specific language(s)
...
# Embed subtitles into video file (MP4/MKV/WebM only)
...
```bash
# Use cookies from your browser (for age-restricted or private videos)
yt-dlp --cookies-from-browser chrome "URL"
yt-dlp --cookies-from-browser firefox "URL"
yt-dlp --cookies-from-browser edge "URL"

# Use cookies from a file (Netscape format)
yt-dlp --cookies cookies.txt "URL"

# Login with username/password (site-specific)
...
# Include video ID in filename (prevents overwrites)
...
# Write video description to a separate file
...
# Resume partially downloaded files (enabled by default)
yt-dlp -c "URL"

# Simulate download (don't actually download, just show info)
yt-dlp -s "URL"

# Show verbose output for debugging
yt-dlp -v "URL"

# Ignore errors and continue (useful for playlists)
yt-dlp -i "PLAYLIST_URL"

# Download only if not already in archive file
yt-dlp --download-archive downloaded.txt "URL"

# Concurrent fragment downloads (faster for DASH/HLS streams)
yt-dlp -N 4 "URL"

# Download specific section of video (requires ffmpeg)
...
yt-dlp supports **thousands of websites**. Below is a categorized list of popular supported sites. For the complete list, see the official [Supported Sites](https://github.com/yt-dlp/yt-dlp/blob/master/supportedsites.md) documentation or run `yt-dlp --list-extractors`.

## Video Platforms
YouTube, Vimeo, Dailymotion, Rumble, BitChute, Odysee/LBRY, PeerTube, Streamable, Wistia, Vidyard, JW Platform, Brightcove, Kaltura

## Social Media
Facebook, Instagram, TikTok, Twitter/X, Reddit, Tumblr, VK, Weibo, Bluesky, Threads, Mastodon, Gab, Gettr, Parler, Minds

## Live Streaming
Twitch, Kick, YouTube Live, Facebook Live, Instagram Live, TikTok Live, Periscope, Livestream, DLive, Trovo, AfreecaTV, Chzzk, Niconico

## Streaming Services (may require authentication)
Amazon Prime Video, Netflix, Disney+, HBO Max, Hulu, Paramount+, Peacock, Crunchyroll, Funimation, Apple TV+, BBC iPlayer, ITV, Channel 4, All 4, BritBox, Stan, Crave

## Music & Podcasts
SoundCloud, Bandcamp, Spotify (limited), Apple Music/Podcasts, YouTube Music, Mixcloud, Audiomack, Deezer (limited), iHeartRadio, TuneIn, Spreaker, Podbean, Anchor, Stitcher

## News & Media
BBC, CNN, Fox News, NBC, CBS, ABC, Al Jazeera, Reuters, AP, The Guardian, Washington Post, New York Times, Bloomberg, CNBC, NPR, PBS

## Educational
Coursera, Udemy, Khan Academy, LinkedIn Learning, Skillshare, PluralSight, edX, MIT OpenCourseWare, TED Talks, Nebula, CuriosityStream, Gaia, MasterClass

## Sports
ESPN, NFL, NBA, MLB, NHL, Formula 1, UFC, WWE, PGA Tour, Eurosport, DAZN, Sky Sports

## Regional/International TV
- **Germany**: ARD, ZDF, ProSiebenSat.1, RTL, 3sat, ARTE
- **France**: France TV, TF1, Canal+, ARTE
- **UK**: BBC, ITV, Channel 4, Sky
- **Spain**: RTVE, Atresplayer, Mitele
- **Italy**: Rai, Mediaset
- **Netherlands**: NPO, RTL
- **Japan**: NHK, AbemaTV, Niconico, TVer, Bilibili
- **South Korea**: KBS, MBC, SBS, Naver TV, Kakao TV
- **China**: Bilibili, Youku, iQiyi, Tencent Video, Douyin
- **India**: Hotstar, SonyLIV, ZEE5, JioCinema, Voot
- **Australia**: ABC iview, SBS On Demand, 7plus, 9Now, 10 Play
- **Canada**: CBC Gem, CTV, Global TV

## File Hosting & Cloud
Dropbox, Google Drive, OneDrive, Mediafire, MEGA (limited), Archive.org, Internet Archive

## Adult Content (18+)
PornHub, XVideos, xHamster, RedTube, YouPorn, and many others

## Other Notable Sites
Archive.org, Imgur (video), Flickr (video), Coub, 9GAG, eBaum's World, Newgrounds, DeviantArt, ArtStation, Behance, Patreon (with auth), Gumroad

---

**Note**:
- Some sites require authentication via `--cookies-from-browser` or login credentials
- Availability and functionality may change as websites update their systems
- Geo-restricted content may require VPN or proxy (`--proxy`)
- Run `yt-dlp --list-extractors` for the complete current list
- Run `yt-dlp --extractor-descriptions` for descriptions of each extractor
